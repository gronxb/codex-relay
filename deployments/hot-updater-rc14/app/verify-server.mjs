...
const request = async (url, apiKey) => {
  ...
  for await (const chunk of response.body ?? []) {
    ...
    requireCheck(size <= maxCatalogWireBytes, "Server response exceeds the probe limit.");
...
try {
  ...
  requireCheck(
    isText(values["base-url"]) &&
      ...
      (values["app-version"] === undefined) !== (values.fingerprint === undefined),
  ...
  requireCheck(
    isText(manifest.serverVersion) && Number.isInteger(manifest.infrastructureGeneration),
  ...
  const localKey = await readFile(new URL("./api-key.local", import.meta.url), "utf8").catch(
    (error) => {
  ...
  requireCheck(
    versionBody?.version === manifest.serverVersion &&
      versionBody?.infrastructureGeneration === manifest.infrastructureGeneration,
