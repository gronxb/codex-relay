...
try {
  ...
  if (apiKey && existingApiKey && apiKey.trim() !== existingApiKey) {
    throw new Error("Saved client keys differ. Resolve the target key before provisioning.");
  }
  if (!apiKey) {
  ...
  console.log(`Client key registered: ${result.record.id}. Key saved in api-key.local.`);
} finally {
